...
test_that("Mable fitted values", {
  ...
  expect_equal(unique(fits[[key_vars(fits)]]), c("fdeaths", "mdeaths"))
  expect_equal(fits[["index"]], UKLungDeaths[["index"]])
  # https://github.com/tidyverts/tsibble/issues/56
...
test_that("Mable residuals", {
  ...
  expect_equal(unique(resids[[key_vars(resids)]]), c("fdeaths", "mdeaths"))
